...
internal class PackStreamListValueTests
{
    ...
    [Test]
    public void EnumeratorThrowsOnUnexpectedEndOfData()
    {
        var decoder = new SingleByteIntDecoder();
        var data = new ReadOnlySequence<byte>([0x01, 0x02]);

        // Claim 3 items but only have data for 2
        var listValue = new PackStreamListValue(data, 3, decoder);

        Action act = () =>
        {
            foreach (var _ in listValue) { }
        };

        act.Should().Throw<InvalidOperationException>()
            .WithMessage("*end of data*");
    }

    [Test]
    public void EnumeratorThrowsOnZeroBytesConsumed()
    {
        var decoder = new ZeroBytesConsumedDecoder();
        var data = new ReadOnlySequence<byte>([0x01]);
        var listValue = new PackStreamListValue(data, 1, decoder);
        ...
        act.Should().Throw<InvalidOperationException>()
            .WithMessage("*zero bytes consumed*");
    }

    [Test]
    public void EnumeratorThrowsWhenConsumedExceedsRemaining()
    {
        var decoder = new ExcessiveBytesConsumedDecoder();
        var data = new ReadOnlySequence<byte>([0x01]);
        var listValue = new PackStreamListValue(data, 1, decoder);
        ...
        act.Should().Throw<InvalidOperationException>()
            .WithMessage("*100 bytes*only*remain*");
    }

    [Test]
    public void ToEnumerableThrowsOnUnexpectedEndOfData()
    {
        var decoder = new SingleByteIntDecoder();
        var data = new ReadOnlySequence<byte>([0x01, 0x02]);
        ...
        Action act = () => listValue.ToEnumerable().ToList();
    ...
    [Test]
    public void ToEnumerableThrowsOnZeroBytesConsumed()
    {
        var decoder = new ZeroBytesConsumedDecoder();
        var data = new ReadOnlySequence<byte>([0x01]);
        var listValue = new PackStreamListValue(data, 1, decoder);
    ...
    [Test]
    public void ToEnumerableThrowsWhenConsumedExceedsRemaining()
    {
        var decoder = new ExcessiveBytesConsumedDecoder();
        var data = new ReadOnlySequence<byte>([0x01]);
        var listValue = new PackStreamListValue(data, 1, decoder);
