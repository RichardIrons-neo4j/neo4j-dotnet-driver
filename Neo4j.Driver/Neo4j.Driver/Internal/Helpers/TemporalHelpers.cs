...
internal static class TemporalHelpers
{
    ...
    public static long ToNanoOfDay(this IHasTimeComponents time)
    {
        return time.Hour * NanosPerHour +
            time.Minute * NanosPerMinute +
            time.Second * NanosPerSecond +
            time.Nanosecond;
    }
    ...
    {
        return FloorMod(duration.Seconds, SecondsPerDay) * NanosPerSecond + duration.Nanos;
    }
    ...
    private static void ComponentsOfNanoOfDay(
        long nanoOfDay,
        out int hour,
        out int minute,
        out int second,
    ...
    public static void AssertNoTruncation(IHasTimeComponents time, string target)
    {
        if (time.Nanosecond % NanosecondsPerTick > 0)
        {
            throw new ValueTruncationException(
                $"Conversion of this instance ({time}) into {target} will cause a truncation of {time.Nanosecond % NanosecondsPerTick}ns.");
        }
    }
    ...
    public static string ToIsoTimeZoneOffset(int offsetSeconds)
    {
        ...
        return offset.Seconds != 0
            ? $"{sign}{offset.Hours:D2}:{offset.Minutes:D2}:{offset.Seconds:D2}"
