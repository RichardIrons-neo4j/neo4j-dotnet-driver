...
internal static class NonGenericCollectionExtensions
{
    private const string DefaultItemSeparator = ", ";

    extension(IDictionary dict)
    {
        public string ToDelimitedString(string separator = DefaultItemSeparator)
        {
    ...
    extension(IEnumerable enumerable)
    {
        public string ToDelimitedString(string separator = DefaultItemSeparator)
        {
            var listStrings = enumerable.Cast<object>().Select(item => item.ToContentString());
