...
public class BoltHandshakerTests
{
    ...
    private async Task DoHandshakeAsyncShouldSelectBoltVersionInRange()
    {
        ...
        var inputData = new byte[]
        {
            ...
            0x00, minorVersionPlus, minorVersionPlus,
            majorVersion, //set to higher than actually supported. latest version + 3 minors -> latest version first major release (e.g. 6.3 -> 6.0 where 6.0 is the current highest supported version). 
            0x00 //no capability flags set
        };
    ...
    [InlineData(
        ...
        {
            0x00, 0x00, 0x02,
            0xFF, //Identifies this as a modern negotiation with manifest v2 which is not known - ERROR
            0x01, //1 versions to follow
            0x00, 0x00, 0x07, 0x05, //Supports version 5.7
            0x00 //no capability flags set
        })]
